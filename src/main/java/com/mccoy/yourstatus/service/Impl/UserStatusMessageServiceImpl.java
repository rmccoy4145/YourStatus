...
public class UserStatusMessageServiceImpl extends AbstractJpaService<UserStatusMessage> implements GenericDAO<UserStatusMessage> {
    ...
    public List<UserStatusMessage> getFollowedMessageFeed(User user) {
        ...
        List<UserFollow> followedUsers = getEm().createQuery("SELECT f FROM UserFollow f WHERE f.user = user", UserFollow.class)
                .getResultList();
        followedUsers.forEach(userFollow -> {
            allFollowersMessages.addAll(getAllByUser(userFollow.getFollowUser()));
        });
